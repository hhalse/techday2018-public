...
class TaskOrientationActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        textViewDescription.setText(buildDescriptionSpannable(), TextView.BufferType.SPANNABLE)
    }

    private fun buildDescriptionSpannable(): SpannableString {
        val activity = "Activity"
        val activityLayoutFileName = "activity_orientation.xml"
        val resourceFileName = "orientation_task_content.xml"
        val resFolder = "res"
        val layoutFolderPath = "res -> layout"
        val linearLayout = "LinearLayout"
        val imageView = "ImageView"
        val textView = "TextView"

        var description = "In this task you will learn how to adjust your layout to fit different configurations."
        description += " The files we're going to look at are located inside the $layoutFolderPath folder of your project."
        description += "\n\nThis Activity (screen) uses $activityLayoutFileName as its main layout file. If you take a look inside this file, you will find that $resourceFileName is included."
        ...
        highlight(spannable, description, activity)
        highlight(spannable, description, activityLayoutFileName)
        highlight(spannable, description, resourceFileName)
        highlight(spannable, description, resFolder)
        highlight(spannable, description, layoutFolderPath)
        highlight(spannable, description, linearLayout)
        highlight(spannable, description, imageView)
        highlight(spannable, description, textView)
